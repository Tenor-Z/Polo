...
#To run this game, please refer to the HOW-TO-RUN file attached.
#--------------------------------------------------------
...
#References is not needed, it's just debugging stuff, but it's a good idea to keep the backup.py file in case this file gets corrupted. Maingame.mp3 is needed since the game pulls this file from the selected directory (the root)
...
#---------------------------------------------------
# This is where we declare our modules that will be used for the game
#------------------------------------------------------
...
#For some reason, the pygame mixer doesn't load properly in replit, so music and sound effects do not work unless you are using IDLE Python
#--------------------------------------------------
...
maingame = pygame.mixer.Sound('maingame.wav')

#------------------------------------------------
# This is a list of global variables that the game
# refers to several times in the game
#------------------------------------------------
...
global intro  #makes sure that an intro is playing
intro = "NO"    #an intro won't be playing on startup


global level   #Indicates what level the game is on
level = 1      #Set it to the first level

#This variable goes unused, however it turns up in the reloadgraphics function in rungame, so I guess it gets used somewhere.
global direction
direction = "Down"
...
#EDIT: 1/19/2022 THIS BUG WAS FIXED!
...
#nonotification lets the player know that they don't have any character introductions to view. This is like viewing messages on an email inbox. Conti, notification and view serve the same purpose, they're just different messages
...
#This is the tip that appears on the Get Ready screen.    Nice Jackal reference!
...
#Several hexadecimal values for different colors used in pygame.
white = (255, 255, 255)
...
#Small font is for handling smaller font
...
def levelcompleted(gameon, level):
  pygame.mixer.music.load("music/popup.wav") #load the music
  pygame.mixer.music.play(0) #And play it (don't loop)
  ...
  lc = pygame.image.load("images/flavor_text/lc.png") #load the level complete text
  lc = pygame.transform.scale(lc, (400,400)) 
  #resize the picture
  #load each different inbox message
...
def ready(gameon):
  ...
  sayit = font.render(str(reminder),1,black) #render the reminder text with the font into a variable for blitting
...
#Tl;dr, it's basically an error handling function that rarely gets used in-game
#-------------------------------------------------------

def msghandler(messagetype):
  #The messagebox variable holds the message BOX that holds the actual message
  messagebox = pygame.image.load("images/msgbox.png") #load the messagebox template
  ...
  if messagetype == "DENIED":
    ...
    pygame.mixer.music.load("music/popup.wav") #play sound
    pygame.mixer.music.play(0)
    #Blit error message
    ...
    #This should do nothing, and everything but anything else
    print("")
    ...
    for event in pygame.event.get():#Capture pygame events
      if event.type == pygame.KEYDOWN:#Check if a key was pressed
        if event.key == pygame.K_SPACE: #Was it space?
          ...
          #They will be commented out so the player has no access to them!
...
def introprinter(level, lives, livemessage, gameon, gathered):
  #global these variables from before
  global intro, messagetype
  pygame.mixer.music.load("music/instruction.wav") #load the stage music and play it.
  pygame.mixer.music.play(0)
...
#
#It pretty much contains all the "game" aspects of the game, so everything important is located here.
#----------------------------------------------------
...
#Gameon, level, gathered and messagetype are needed to pass current information to other functions
def rungame(gameon, level, gathered, messagetype):
 global power, faster, youfaster, kill, event, lives
 #Global these variables to be used with or without functions
 
 #Load the correct music file and play it (loop)
 pygame.mixer.music.load("maingame.wav")
 ...
 # 6. Player loses a life  (UNUSED)
 #-------------------------------------------------
 
 #Pick a random number to generate the maze (only comes into play if the player has completed 5 levels)
 mazepick = random.randint(1,6)
 print("Mazepick selected " + str(mazepick)) #debug for debugging
 event = 0  #No current events going on
 
 #---------------------------------------------------
 #These are border attributes used in case the enemies go off screen, so they can respawn again. They aren't viewable to the player.
 ...
 #Is the level 4?
 #NOTE: Viewing the cutscene counts as beating the level, but the player doesn't know, so shhhhhh
 if level == 4:
   intro = "YES"   #Start the next intro
 else:
   print("")

 #-------------------------------------------------
 # The character introduction post level 4 gets blitted here
 #-------------------------------------------------
 #If the level is 4?
 if level == 4:
   if intro == "YES":       #Set the intro to yes
    livecounter = font.render(str(lives),1,white)
    sayit = font.render(str(livemessage),1,white) 
    tip = pygame.image.load("images/intro2.png")
    gameWindow.blit(tip, (0,0))     #Blit everything, like before
    gameWindow.blit(livecounter,(310, 50))   #to their
    gameWindow.blit(sayit,(150, 50))  #x and y values
    pygame.display.flip()
    pygame.time.wait(7000)
    intro = "NO"        #Switch it to no so the loop shuts off
    gameon = "Level Completed!"
    levelcompleted(gameon, level)    #return
   else:
     intro = "NO"       

 #------------------------------------------
 # Here are some variables specific to rungame
 #------------------------------------------ 
 #Global the origin X and Y coordinates
 ...
 #Use this for debugging, otherwise leave it commented
 ...
 vac = pygame.transform.scale(vac,(100,100))
 # --------------------------------------------
 ...
 if level == 2 or level == 3 or level == 4:
   Xorigin = 60   #Set different positions depending on where the black walls are located
   Yorigin = 60
 if level == 5:
   Xorigin = 520
   Yorigin = 200
 ...
 playerRect = player.get_rect() #Make rectangle for collision
 playerW = playerRect.width
 playerH = playerRect.height

 #=========================================
 #      The following variables are for the vaccine,       which hold some similarities to player variables 
 #------------------------------------------
 vaccineRect = vac.get_rect() #Make the rectangle
 vaccineW = vaccineRect.width
 vaccineH = vaccineRect.height
...
#------------------------------------------------
 ...
 #---------------------------------------------
 #All characters have their own rect so they can behave differently on screen simutanously.
 #
 # This is for gelloleg   (gelloleg is gello both forwards and backwards)
 #-----------------------------------------------
 ...
 #------------------------------------------------
 #    Here is Don's variable collection
 #     Named after my favorite Ninja Turtle of course
 #------------------------------------------------
 ...
 #--------------------------------------------
 #Here is the exit's variable collection
 #--------------------------------------------
 ...
 #Pick a random maze when the level is greater than 6
 mazepick = random.randint(1,6)

 #This is the variable that holds the random situation when in contact with the powerup
 #I figured I already made this game hard enough, so why not make the powerup random as well.
 ...
 def reloadgraphics(event, power):    
    ...
    if level == 1 and kill == 0 or not level == 1 and kill == 0:
      ...
      if power == 0: #has the power not been collected yet
       ...
       #If the level is higher than 4 and the enemies aren't killed
    if level > 4 and kill == 0:
      gameWindow.blit(gelloleg,(gellX,gellY)) #Blit these two
 ...
 #---------------------------------------------
 # LOAD EVERY LEVEL AND WALL COLLISION HANDLER
 #----------------------------------------------
 ...
 #---------------------------------------------
 # This loops while the game is running
 #----------------------------------------------
 while gameon == "game loaded":  #While the game is loaded?
   ...
   # easier.
   #
   # NOTE: I made the characters go through walls to make
   # it more difficult
  
   #THIS AI SYSTEM MOVES THE ENEMY DEPENDING ON IT'S X AND Y COORDINATES
   #------------------------------------------------
...
#-------------------------------------------
    ...
    #==================================================
    # Here is where the game detects button presses
    #==================================================
    ...
    #=================================
    #Here is a collection of the rects of several sprites
    #They are used to detect collision
    #-======================================    
    playerRect = pygame.Rect(ballX,ballY,playerW,playerH)
    vaccineRect = pygame.Rect(resultX,resultY,vaccineW,vaccineH)
    #Did the vaccine collide with the player
    if vaccineRect.colliderect(playerRect):
        gathered += 1  #Set to gathered
        pygame.mixer.music.load("music/collected.wav")
        pygame.mixer.music.play(0) #play the sound

    exitRect = pygame.Rect(exitX,exitY,exitW,exitH)

    #Did the exit collide with the player?
    if exitRect.colliderect(playerRect):
      if gathered > 0:  #if vaccine was already collected
        if level == 70:  #if level is 70
          ending()    #show the ending
        else:  #If it isn't
          print("Exit enabled...")
          print("Level complete...")
          gameon = "Level Completed!"   #show the level was completed
          levelcompleted(gameon, level)
      else:   #If the vaccine wasn't collected
        messagetype = "DENIED"  #Deny access
        msghandler(messagetype)

    #----------------------------------------------
    # Here is a collection of collision rects of enemies and the invisible borders
    #----------------------------------------------    
    ...
    # did Repla collide with the player
    if replaRect.colliderect(playerRect):
        ...
        return(lives)   #return lives so it doesn't forget

     #did Cate collide with Player?   
    if cateRect.colliderect(playerRect):
        print("died!")  #debug message: the cursed child
        #lives -= 1  #decrease lives by 1
        pygame.mixer.music.load("music/die.wav")
        pygame.mixer.music.play(0)
    ...
    #Did Gellolleg collide with player?
    if gellRect.colliderect(playerRect):
        print("died!")  #debug message: the cursed child
        #lives -= 1  #decrease lives by 1
        pygame.mixer.music.load("music/die.wav")
        pygame.mixer.music.play(0)
    ...
    #Did Don collide with player?
    if donRect.colliderect(playerRect):
        print("died!")  #debug message: the cursed child
        #lives -= 1  #decrease lives by 1
        pygame.mixer.music.load("music/die.wav")
        pygame.mixer.music.play(0)
    ...
    #--------------------------------------------
    # Here is where the border handles the possibility
    # where enemies go off screen. They will always appear
    # in their starting position
    #-----------------------------------------------
    if borderRect.colliderect(replaRect):
      replaX = 120    #Repla's starting position
      replaY = 120
    ...
    if borderRect.colliderect(gellRect):
      gellX = 350    #Gell's starting position
      gellY = 120
    ...
    if borderRect.colliderect(donRect):
      donX = 230    #Don's starting position
      donY = 300
    ...
    #NOTE: Cate wasn't added because his border collision and reset position was already implemented days before this script was added
      
    #Did the player collide with the powerup
    powRect = pygame.Rect(powerX,powerY,powerW,powerH)
    if powRect.colliderect(playerRect):
      print("Powerup obtained!!")    #debug message
...
def died(ballX, ballY, Xorigin, Yorigin, gameon):
  global lives   #global it so we can decrease it
  lives -= 1
  pygame.mixer.music.load("music/static.wav") #play static
  pygame.mixer.music.play(0)
  ...
  gameWindow.blit(static,(0,0)) #loop static "animation"
  pygame.display.flip()
    ...
    for event in pygame.event.get():#Capture pygame events
      if event.type == pygame.KEYDOWN:#Check if a key was pressed
        if event.key == pygame.K_ESCAPE: #Was it space?
          command = input("Please enter your name!")
          f = open("Scores.txt", "x")   #Put it into a file
          f.write((command) + "reached level " + str(level)) # write to file
          print("escape was pressed") #A debugging thing
          exit()

#-------------------------------------------------
#This is the function that runs after level 70 is completed
#-------------------------------------------------
def ending():
  hang = False #set to false cause we're not looping anything
...
#-----------------------------------------
# Here is the function that holds the second
# cutscene
#------------------------------------------
...
title(gameon, start)  #ALWAYS RUN THE TITLE AT THE START